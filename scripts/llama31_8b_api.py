"""Llama 3.1 8B API calls, retry handling, and raw response logging."""
...
class Llama31_8BClient:
    """Together AI Llama 3.1 8B client with SafeAlert retry settings."""
    ...
    def __init__(
        ...
        resolved_api_key = api_key or os.getenv(TOGETHER_API_KEY_ENV)
        if not resolved_api_key:
            raise ValueError(f"{TOGETHER_API_KEY_ENV} is not set.")
    ...
    def call_prompt(self, prompt_text: str, system_prompt: str | None = None) -> APIResult:
        """Call Llama 3.1 8B for one prompt and return a raw API result."""
        last_error = ""
...
def run_prompts(
    ...
    run_type: str,
    system_prompt: str | None = None,
    client: Llama31_8BClient | None = None,
) -> list[dict[str, Any]]:
    """Run a prompt DataFrame through Llama 3.1 8B and log raw JSONL records."""
    api_client = client or Llama31_8BClient()
    records = []
...
def main() -> None:
    """Run Llama 3.1 8B against a prompt dataset and write raw JSONL output."""
    parser = argparse.ArgumentParser(description="Run Llama 3.1 8B and log SafeAlert raw responses.")
